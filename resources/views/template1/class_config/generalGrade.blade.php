            ...
            <div class="tab-content" id="pills-tabContent">
                ...
                <div class="tab-pane fade show active" id="pills-general-grade" role="tabpanel"
                    ...
                    <div class="d-flex justify-content-between">
                        <div class="mt-3">
                            <h4 class="card-title" style="color:rgba(0, 0, 0, 0.5)">General Grade List</h4>
                            <p class="card-description"></p>
                        </div>

                        <a class="btn btn-primary pt-2 mr-2" id="addBtn" style="width: 125px;height: 34px;"
                    ...
                    <div class="">
                        <table id="customTable" class="table table-bordered table-responsive" style="width:100%">
                            ...
                            <tbody>
                                @foreach ($generale_grades as $key=>$g)
                                <tr>
                                    <td> {{ $g->gpa_name }} </td>
                                    <td> {{ $g->range_from }} </td>
                                    <td> {{ $g->range_to }} </td>
                                    <td> {{ $g->gpa_point }} </td>
                                    <td width='9%'>
                                        <a href="javascript:void(0)" onclick="editBtn({{$g->id}})" class="btn btn-sm btn-primary p-1"><i style="margin-left: 0.3125rem;" class="fa-solid fa-pen-to-square"></i></a>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-body">
                <form action="{{route('genGrade.store')}}" method="POST">
                    @csrf
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="groupModal" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-body">
                <form action="{{route('genGrade.update',1)}}" method="POST">
                    @csrf
...
<script>
    ...
    {
        $.ajax({
            url     : '/academic/class/general-grade/edit/'+id,
            type    : 'Get',

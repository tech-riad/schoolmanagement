        ...
        $.each(data.subjects,function (i,v){
            ...
            $.each(v,function (i,v){
                ...
                $.each(data.subjects_types,function (index,value){
                    subjectTypes += `<input class="form-check-input" ${v.type_id == value.id ? 'checked':''}  title="${value.name}" type="radio" name="subject_id-${v.id}" value="${value.id}-${v.id}" id="subject_id-${v.id}" >`;
                });
                trow += `<td>
